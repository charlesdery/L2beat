...
base_path = os.getcwd()
output_path = r"C:\Users\charl\Google Drive\2021\investing_programs\AWS Serverless Application\L2beat" + "\output"
...
df = df[['Date','Name', 'Value LockedTVL', '7 days change% / 7 days', 'Market share','Purpose','TechnologyTech','Source','Timestamp']]  
...
s3.Bucket(s3_bucket).upload_file(Filename = output_path + '\\' + power_bi_file, Key=  s3_upload + power_bi_file, ExtraArgs={'ACL':'public-read'})

print("PolygonScan transaction data loaded successfully")
print(df.head())
sys.exit()